#!/usr/bin/env bash
# execute_chat_app.sh — launch the Gradio delivery chat UI.
#
# Thin wrapper, same pattern as execute_topology.sh and execute_experiment.sh: pins the venv
...
# scripts/execute_experiment.sh for a batch.
#
...
#
# Usage:
#   ./execute_chat_app.sh                  # start the UI on http://127.0.0.1:7860
#   SC_NO_CACHE=1 ./execute_chat_app.sh    # disable response cache and freshness reuse
set -euo pipefail
