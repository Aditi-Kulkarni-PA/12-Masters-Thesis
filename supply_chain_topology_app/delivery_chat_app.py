...
async def chat_handler(message: str, history: list, orders_path, pending_query: str, tab_state: dict):
    ...
    def _is_real(text: str) -> bool:
            """True if *text* is actual output (not empty / 'not run' / a pipeline error)."""
            if not text:
                return False
            return not is_error_text(text)
    ...
    def _apply_payload(tool_name: str, summary: str = "", reapply: bool = False) -> bool:
        ...
        if tool_name == "predict_delivery_delays_tool":
            ...
            if is_error_text(predict_text):
                run_warnings.append(f"predict_delivery_delays_tool: {predict_text.strip()}")
        ...
        # --- DIAGNOSE TOOL HANDLER ---
            ...
            if is_error_text(diagnosis_text):
                run_warnings.append(f"diagnose_delay_patterns_tool: {diagnosis_text.strip()}")
            ...
            if diagnosis_text and not is_error_text(diagnosis_text):
