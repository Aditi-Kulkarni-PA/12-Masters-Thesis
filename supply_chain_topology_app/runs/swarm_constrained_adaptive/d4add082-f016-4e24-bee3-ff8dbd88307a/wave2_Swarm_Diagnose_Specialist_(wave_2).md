...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to the shared blackboard the diagnosis results for today: for each predicted delayed order (or for each high-risk pattern group, if the system aggregates), identify the most likely delay reasons/pattern drivers by comparing against historical baselines. Include the pattern dimensions involved (as available) and the rationale signals (e.g., which baseline comparisons were abnormal). This is required to answer the 'reasons for the delay' part of the request.
