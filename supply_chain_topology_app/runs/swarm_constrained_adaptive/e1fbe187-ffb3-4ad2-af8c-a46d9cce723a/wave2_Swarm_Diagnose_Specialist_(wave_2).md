...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Using today’s predicted delayed-orders output from the blackboard, analyze those delays against historical baselines across all relevant dimensions and identify the high-risk pattern combinations driving the delays. Post the diagnosis results (pattern drivers, risk factors, and any relevant breakdowns) to the shared blackboard. This matters because it answers the “what patterns are driving those delays” part and provides evidence for recommendations.
