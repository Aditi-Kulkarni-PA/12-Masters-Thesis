...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Using today’s predicted delayed-orders output from the blackboard, generate severity-based customer email alerts: one personalized message per affected order. Ensure each email is tied to the correct order/customer and reflects the predicted severity level. Post the full set of generated customer email messages (with order/customer linkage) to the shared blackboard. This matters because it fulfills the “notify the affected customers” requirement.
