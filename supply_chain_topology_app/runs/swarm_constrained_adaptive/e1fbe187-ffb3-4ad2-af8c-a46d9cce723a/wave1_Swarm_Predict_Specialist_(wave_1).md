...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the delay prediction pipeline on the provided raw input orders file and produce today’s predicted delayed orders with severity classification for each affected order. Post the resulting predicted delayed-orders dataset (including order identifiers and predicted severity) to the shared blackboard so downstream steps can diagnose patterns and generate customer alerts. This matters because it is the required input for diagnosis, simulation (if needed), recommendations, and email alerts.
