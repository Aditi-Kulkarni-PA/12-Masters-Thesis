...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders with severity classifications to the shared blackboard, including any tool-level error exactly if prediction cannot be completed, because both the diagnosis and the East storm simulation depend on this output.
