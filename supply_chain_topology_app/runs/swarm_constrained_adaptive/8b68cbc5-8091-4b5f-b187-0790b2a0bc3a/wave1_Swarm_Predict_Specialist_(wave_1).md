...
- capability: `predict`
- wave: 1
- run: `8b68cbc5-8091-4b5f-b187-0790b2a0bc3a`
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML prediction over the provided input orders file and identify which of today's orders are predicted to be delayed, including severity classification for each affected order. Post to the shared blackboard the complete set of today's predicted delayed orders (with order identifiers and predicted severity) so downstream steps (if any) can use it. This matters because the user asked specifically for the predicted delayed orders for today.
