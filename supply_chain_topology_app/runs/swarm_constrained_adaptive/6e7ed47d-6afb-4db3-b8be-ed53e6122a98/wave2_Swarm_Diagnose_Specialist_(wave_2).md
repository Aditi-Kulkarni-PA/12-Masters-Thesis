...
- capability: `diagnose`
- wave: 2
- run: `6e7ed47d-6afb-4db3-b8be-ed53e6122a98`
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to the shared blackboard the diagnosis results explaining the reasons for delay for TODAY’s predicted delayed orders. Include: (1) the high-risk pattern combinations found by comparing predicted delays to historical baselines (by dimensions such as region/route, carrier/vehicle type, delivery mode, time-of-day/day-of-week, and any other available dimensions), and (2) a mapping from each affected order (or each pattern group) to the most likely delay drivers/patterns. This matters because the user asked not only which orders will be late, but also the reasons for the delay.
