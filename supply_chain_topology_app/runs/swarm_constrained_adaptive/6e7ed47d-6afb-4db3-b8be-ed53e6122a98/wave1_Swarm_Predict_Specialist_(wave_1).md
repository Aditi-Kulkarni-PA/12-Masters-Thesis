...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to the shared blackboard the full set of TODAY’S predicted delayed orders from the provided file, including for each affected order: predicted delay status (delayed/not), predicted severity class, and any key features/fields your pipeline uses that are necessary for downstream diagnosis (e.g., route/region, carrier/vehicle attributes, timestamps, and the model’s relevant contributing signals if available). This matters because diagnose must compare these predicted delays against historical baselines to identify the reasons/patterns.
