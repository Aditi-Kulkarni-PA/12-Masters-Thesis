...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to the shared blackboard the full set of TODAY’s predicted delayed orders from the provided input file, including for each affected order: predicted delay indicator (delayed/not), predicted severity classification, and any key fields needed downstream (e.g., order identifier and the dimensions used for later pattern analysis). This matters because every subsequent step (diagnosis, simulation, recommendations, and customer emails) must be grounded in the model’s predicted delays for today.
