...
Before you finish, you MUST call write_blackboard to post your result. What to post: Produce data-driven optimization recommendations across quick-win, short-term and long-term horizons. Works from today's predicted delay output together with today's diagnosis results, and from retrieved service-level knowledge. Post the recommendations (grouped by quick-win / short-term / long-term, tied to the diagnosed pattern drivers and predicted delayed orders) to the shared blackboard so the delivery manager can act on them.
