...
Before you finish, you MUST call write_blackboard to post your result. What to post: Generate severity-based customer email alerts, one personalised message per affected order. Works from today's predicted delayed orders. Post the customer email alerts (per affected order, including the recipient and message content) to the shared blackboard so customer notifications can be sent for the predicted delayed orders.
