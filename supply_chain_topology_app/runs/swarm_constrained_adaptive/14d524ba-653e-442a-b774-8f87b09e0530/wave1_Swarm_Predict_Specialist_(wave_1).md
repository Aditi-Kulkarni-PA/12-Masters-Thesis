...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML pipeline over the raw input orders file supplied in the request, to predict which of today's orders will be delayed and classify each one's severity. Produce today's predicted delayed orders. Needs nothing but the input orders file. Post the predicted delayed orders (including order identifiers and severity classification) to the shared blackboard so downstream diagnosis, simulation, recommendations, and customer email alerts can use it.
