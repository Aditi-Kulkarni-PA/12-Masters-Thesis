...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to the shared blackboard the predicted delayed orders for today, including for each affected order: predicted delay status, severity classification, and any key fields needed to join downstream outputs (e.g., order identifier and relevant delivery attributes). This is the required input for diagnosis, simulation, recommendations, and customer email alerts.
