...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to the shared blackboard the predicted delayed orders for today, including for each affected order: order identifier, customer identifier (if present), customer email (if present), predicted delay status, and severity classification. This is required so the system can identify impacted customers and generate severity-based customer email alerts.
