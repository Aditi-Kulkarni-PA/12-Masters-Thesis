...
- capability: `predict`
- wave: 1
- run: `efd2703a-2385-4e9a-8a6c-1c0ec988a7d8`
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the model’s output for today: the list of predicted delayed orders from the provided raw file, including for each affected order the fields needed downstream to identify the customer and severity (e.g., order identifier, customer identifier/email if present in the data, predicted delay/severity classification, and any delivery date/time context). This is required so the system can generate customer-specific delay impact and severity-based email alerts.
