...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the delay prediction pipeline on the supplied input orders file and post today's predicted delayed orders, including severity/classification for each affected order, to the shared blackboard. This is needed because the request asks which orders will be late, and downstream diagnosis can only explain reasons for delay from the prediction output.
