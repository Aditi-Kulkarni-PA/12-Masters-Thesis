...
- wave: 3
- run: `38216e11-ace1-4ebf-902f-33e4df48113d`

## Task (user turn)

Produce data-driven optimization recommendations across quick-win, short-term and long-term horizons using today's predicted delay output already produced from the input orders file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv and today's diagnosis results. Focus on the reasons behind the predicted late orders and the root-cause patterns behind the prediction output.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post recommendation results to the blackboard, using today's predicted delay output and today's diagnosis results to generate data-driven optimization recommendations across quick-win, short-term and long-term horizons. This is needed because the request asks for reasons behind the predicted late orders, and recommendations turn those diagnosed root-cause patterns into actions.
