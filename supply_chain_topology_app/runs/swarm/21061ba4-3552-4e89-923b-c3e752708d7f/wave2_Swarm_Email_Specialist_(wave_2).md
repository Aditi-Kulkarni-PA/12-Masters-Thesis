...
Generate severity-based customer email alerts, one personalised message per affected order, using today's predicted delayed orders that have now been posted to the blackboard from predictions run on: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv. The user wants customer notifications for affected customers.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the generated customer email alerts to the blackboard, one personalised message per affected delayed order or per supported affected subset, using today's predicted delay output. This is needed because the user explicitly asked for customer notifications for affected customers.
