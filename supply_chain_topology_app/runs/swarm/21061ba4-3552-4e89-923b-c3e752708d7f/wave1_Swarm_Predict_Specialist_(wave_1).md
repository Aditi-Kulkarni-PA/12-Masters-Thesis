...
Predict which orders in today's input orders data are likely to be delayed and classify each delayed order's severity. The input orders data is in the file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv
The user wants: which orders are likely to be delayed today, what patterns are driving those delays, and customer notifications for affected customers.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders and severity classifications to the shared blackboard, including enough identifying detail for downstream diagnosis and customer notification. This matters because diagnosis and email alerts both depend on today's predicted delay output.
