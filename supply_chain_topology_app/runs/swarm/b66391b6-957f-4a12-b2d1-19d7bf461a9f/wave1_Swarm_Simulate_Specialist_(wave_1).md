...
- capability: `simulate`
- wave: 1
- run: `b66391b6-957f-4a12-b2d1-19d7bf461a9f`

## Task (user turn)

What patterns are driving today's delays, and would that get worse if a storm hit the East?
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Simulate a what-if scenario where a storm hits the East, and report how delay severity shifts relative to today. Use today’s predicted delayed orders as the starting point and re-score severity against historical patterns available in the system. Post the storm-vs-today simulation results to the shared blackboard so the delivery manager can judge whether it would get worse.
