...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run delay-pattern diagnosis for today by analyzing the model’s predicted delayed orders against historical baselines across all relevant dimensions, and identify the highest-risk pattern combinations driving today’s delays. Post the diagnosis results to the shared blackboard so downstream steps (simulation and recommendations) can use them.
