...
Use today's orders file to predict which of today's orders will be delayed and classify each order's severity. Then support the user's request for both typical delay patterns and how last-mile delivery can be optimised. The input orders data is in the file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders and severity classifications to the shared blackboard, including enough summary context for downstream specialists to use them. This matters because diagnosis needs today's predicted delay output to identify typical delay patterns, and recommendations need the prediction output as one of their required inputs.
