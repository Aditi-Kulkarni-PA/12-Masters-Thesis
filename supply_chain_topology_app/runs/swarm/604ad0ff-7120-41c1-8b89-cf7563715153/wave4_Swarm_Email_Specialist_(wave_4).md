...
Use today's predicted delay output already produced from the input orders file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv. Generate severity-based customer email alerts, one personalised message per affected order, using today's predicted delayed orders that are already available.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post severity-based customer email alerts generated from today's predicted delayed orders. This is needed because prediction has already been produced and email depends on predict output, which is available.
