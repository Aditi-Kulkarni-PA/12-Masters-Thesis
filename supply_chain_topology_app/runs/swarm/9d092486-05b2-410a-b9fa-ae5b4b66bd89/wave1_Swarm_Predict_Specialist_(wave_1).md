...
Run the two-stage ML pipeline over the raw input orders file supplied in the request to predict which of today's orders will be delayed and classify each one's severity. The input orders data is in the file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv. Scenario wording from the user: "What will be the delivery impact if there is a storm in the east today?" Use this exact scenario context when handing off any downstream work.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders to the shared blackboard, including each affected order and its predicted severity, because downstream scenario analysis needs the current delay predictions as its input.
