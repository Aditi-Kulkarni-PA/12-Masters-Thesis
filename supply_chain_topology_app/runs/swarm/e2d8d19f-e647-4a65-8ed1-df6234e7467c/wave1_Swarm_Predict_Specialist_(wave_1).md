...
- capability: `predict`
- wave: 1
- run: `e2d8d19f-e647-4a65-8ed1-df6234e7467c`

## Task (user turn)

Identify customers impacted with delivery delays and send an email to them
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the delay prediction pipeline on the provided raw input orders file and determine which of today’s orders are predicted to be delayed, including a severity classification per affected order. Post to the blackboard the list of impacted orders (with whatever order identifiers are available in the file) and their predicted delay/severity so the email specialist can generate one personalized customer alert per affected order.
