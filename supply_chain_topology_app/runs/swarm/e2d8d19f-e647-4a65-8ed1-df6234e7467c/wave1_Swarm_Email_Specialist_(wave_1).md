...
Before you finish, you MUST call write_blackboard to post your result. What to post: Generate severity-based customer email alerts, one personalized message per affected order. Use the predicted delayed orders + severity that will be posted to the blackboard by the prediction step, and post the resulting email drafts (mapped to the correct affected orders/customers using the identifiers available) to the blackboard for delivery/dispatch.
