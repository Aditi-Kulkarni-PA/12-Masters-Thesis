...
What are the typical delay patterns and how can we optimize last-mile delivery?
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: After the predicted delayed orders are available on the blackboard, analyze today’s predicted delays against historical baselines across all relevant dimensions, and identify the highest-risk pattern combinations (typical delay patterns). Post the diagnosis results to the shared blackboard so they can be used to produce concrete optimization actions.
