...
Run a what-if simulation for this exact scenario from the user: check how a storm in the East would affect things. Use today's predicted delayed orders already produced from the input orders file. Apply filters for region east and changes for weather_condition stormy. The input orders data is in the file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the simulation results for an East-region storm scenario to the blackboard, including how delay severity shifts under the scenario, because the user explicitly asked how a storm in the East would affect delays.
