...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the prediction pipeline on the supplied input orders file and post today's predicted delayed orders with severity classifications to the shared blackboard, because diagnosis, simulation, and recommendations all depend on the predicted delay output.
