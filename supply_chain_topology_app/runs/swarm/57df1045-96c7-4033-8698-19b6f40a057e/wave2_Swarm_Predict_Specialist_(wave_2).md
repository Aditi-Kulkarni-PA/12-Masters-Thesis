...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to blackboard under capability 'predict' the produced output (today's predicted delayed orders + severity). Include any summary metrics (counts by severity, top dimensions) so downstream recommend/email can cite them.
