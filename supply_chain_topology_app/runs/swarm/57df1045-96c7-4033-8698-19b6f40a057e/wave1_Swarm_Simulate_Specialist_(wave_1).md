...
Diagnose today's delay patterns, check how a storm in the East would affect things, and tell me how to optimize delivery
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Simulate what-if changes for a storm in the East and report how delay severity shifts. Use today's predicted delayed orders as the starting point and re-score severity against historical patterns already available. Post the storm-in-East simulation output (severity shifts / affected orders summary) to the shared blackboard so recommendations can incorporate the impact.
