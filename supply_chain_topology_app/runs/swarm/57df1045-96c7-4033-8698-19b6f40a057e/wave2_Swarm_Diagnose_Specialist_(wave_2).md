...
- capability: `diagnose`
- wave: 2
- run: `57df1045-96c7-4033-8698-19b6f40a057e`

## Task (user turn)

Analyze today's predicted delays against historical baselines across every dimension, and identify high-risk pattern combinations. Works from today's predicted delay output; the historical baseline is already available and needs no work to produce. Produces today's diagnosis results.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to blackboard under capability 'diagnose' the full diagnosis text (verbatim) including all figures (daily vs historical, worst dimensions, hotspots, and pattern combos). Downstream recommend_actions must pass this diagnosis_summary verbatim into the recommendation tool.
