...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the delay prediction pipeline on today's input orders and post today's predicted delayed orders with severity classifications to the shared blackboard, because the request asks which orders may be delayed today and that output is also required by downstream diagnosis, simulation, recommendations, and customer email generation.
