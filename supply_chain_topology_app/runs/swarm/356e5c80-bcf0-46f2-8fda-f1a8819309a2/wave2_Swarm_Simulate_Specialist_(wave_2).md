...
Simulate what will happen if there is a storm in the east region using today's predicted delayed orders already produced from the input orders file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv. Carry the user's scenario wording through exactly: what will happen if there is a storm in the east region.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the east-region storm what-if simulation results to the blackboard, including how delay severity shifts under the scenario, because the user explicitly asked what will happen if there is a storm in the east region.
