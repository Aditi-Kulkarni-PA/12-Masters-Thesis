...
Generate severity-based customer email alerts, one personalised message per affected order, using today's predicted delayed orders. Use the delayed orders output already produced. Keep messages concise, empathetic, and aligned to the predicted severity label and delay_reason.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post a JSON object containing email drafts or structured email content for delayed orders, because downstream response needs customer-facing alerts.
