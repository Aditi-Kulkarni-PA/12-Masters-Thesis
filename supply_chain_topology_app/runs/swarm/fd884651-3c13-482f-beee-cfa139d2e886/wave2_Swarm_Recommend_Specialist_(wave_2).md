...
- wave: 2
- run: `fd884651-3c13-482f-beee-cfa139d2e886`

## Task (user turn)

Produce data-driven optimization recommendations across quick-win, short-term and long-term horizons using today's predicted delay output and today's diagnosis results. Use the posted diagnosis findings from the blackboard. Focus on actions for the current day, especially stormy east-region risk, express mode, long-distance shipments, and partners/categories with worsening delay rates.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post a JSON object with recommendation sections and concrete actions, because downstream response needs optimization guidance grounded in today's diagnosis.
