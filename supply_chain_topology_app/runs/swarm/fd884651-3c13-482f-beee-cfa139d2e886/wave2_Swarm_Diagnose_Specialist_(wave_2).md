...
Analyse today's predicted delays against historical baselines across every dimension, and identify high-risk pattern combinations. Use today's predicted delay output once it is available.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's diagnosis results to the shared blackboard, including daily vs historical comparisons, worse-than-historical dimensions, high-risk patterns, long-severity hotspots, and any storm/east-region relevant findings so recommend and simulate can use them.
