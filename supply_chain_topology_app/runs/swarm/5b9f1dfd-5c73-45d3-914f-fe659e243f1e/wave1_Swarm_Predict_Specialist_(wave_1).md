...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML pipeline over the raw input orders file at the provided path and predict which of today’s orders will be delayed, classifying each delayed order’s severity. Post the resulting dataset of today’s predicted delayed orders (including order identifiers and severity) to the shared blackboard so downstream steps can diagnose reasons.
