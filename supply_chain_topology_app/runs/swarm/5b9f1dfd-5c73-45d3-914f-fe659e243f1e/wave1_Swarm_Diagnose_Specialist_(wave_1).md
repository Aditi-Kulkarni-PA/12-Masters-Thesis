...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Using today’s predicted delayed orders from the blackboard, analyze them against historical baselines across all relevant dimensions and identify the high-risk pattern combinations that explain the delays. Post the diagnosis results (reason/pattern drivers per order or per pattern group, with supporting baseline comparisons) to the shared blackboard.
