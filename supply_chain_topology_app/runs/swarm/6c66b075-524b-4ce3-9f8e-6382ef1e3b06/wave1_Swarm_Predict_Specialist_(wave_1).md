...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the delay prediction pipeline over the provided raw input orders file and produce today’s predicted delayed orders with severity classification for each affected order. Post the resulting dataset (or a blackboard reference to it) to the shared blackboard so the email specialist can identify impacted customers and generate severity-based alerts.
