...
Simulate whether delays would get worse if a storm hit the East. Use today's predicted delayed orders already posted on the blackboard from file /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/processed/daily_delivery_delay_prediction.csv. Scenario context to preserve verbatim: What patterns are driving today's delays, and would that get worse if a storm hit the East? Apply the what-if to East-region orders by changing weather_condition to stormy.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the East-storm what-if simulation results to the blackboard, including the scenario wording, filters/changes used, and how severity shifts, because the user explicitly asked whether today's delay pattern would worsen if a storm hit the East.
