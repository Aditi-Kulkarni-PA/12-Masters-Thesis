...
Determine which of today's orders in the supplied input file are predicted to be delayed and classify each delayed order's severity.
...
Also preserve this request context for any downstream specialist you may request later: What patterns are driving today's delays, and would that get worse if a storm hit the East?
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders and severity classifications to the shared blackboard, including any output locations or identifiers needed by downstream specialists. This is required because diagnosis needs today's predicted delay output, and the storm-in-East what-if may also need the predicted delayed orders.
