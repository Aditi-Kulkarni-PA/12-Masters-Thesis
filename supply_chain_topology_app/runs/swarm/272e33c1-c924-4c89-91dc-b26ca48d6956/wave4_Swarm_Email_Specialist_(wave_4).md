...
Use today's predicted delayed orders already posted on the blackboard from file /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/processed/daily_delivery_delay_prediction.csv to generate severity-based customer email alerts, one personalised message per affected order. Scenario context to preserve: "What patterns are driving today's delays, and would that get worse if a storm hit the East? Apply the what-if to East-region orders by changing weather_condition to stormy." The simulation used filters {"region":"east"} and changes {"weather_condition":"stormy"}. Key simulation result: 241 of 1346 delayed orders affected; original severity dist {"Short (1-2h)":122,"Medium (3-5h)":97,"Long (6+h)":22}; simulated severity dist {"Short (1-2h)":19,"Medium (3-5h)":152,"Long (6+h)":70}; severity worsens materially toward Medium and Long. Use this context to ensure East-region storm-exposed customers receive appropriately cautious messaging.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the generated customer email alerts to the blackboard because prediction output is already available and the East-storm simulation context is already posted, so email messaging is now ready and should reflect the scenario-specific worsening risk where relevant.
