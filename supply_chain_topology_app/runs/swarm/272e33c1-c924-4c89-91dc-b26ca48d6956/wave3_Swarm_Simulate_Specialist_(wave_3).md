...
Preserve this request context: What patterns are driving today's delays, and would that get worse if a storm hit the East? Run a what-if simulation for an East storm scenario using today's predicted delay output already posted on the blackboard from file /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/processed/daily_delivery_delay_prediction.csv. Simulate the impact if weather in the East were stormy by filtering East-region orders and changing weather_condition to stormy.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the East storm what-if simulation results to the blackboard, including scenario description, affected rows, and how delay severity shifts, because the user explicitly asked whether delays would get worse if a storm hit the East and downstream synthesis may need that scenario result.
