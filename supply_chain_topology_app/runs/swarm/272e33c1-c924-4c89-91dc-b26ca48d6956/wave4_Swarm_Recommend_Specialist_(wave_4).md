# Swarm Recommend Specialist (wave 4)
...
Use today's predicted delay output and today's diagnosis results already posted on the blackboard, plus the East storm what-if simulation result now posted on the blackboard, to produce data-driven optimization recommendations for this request context: 'What patterns are driving today's delays, and would that get worse if a storm hit the East?' Include actions that address the diagnosed drivers and pre-emptive mitigations for an East storm scenario.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post recommendation results to the blackboard, using the already-posted predict and diagnose outputs plus the East storm simulation result, because downstream synthesis may need actionable next steps tied to both today's delay patterns and the what-if worsening scenario.
