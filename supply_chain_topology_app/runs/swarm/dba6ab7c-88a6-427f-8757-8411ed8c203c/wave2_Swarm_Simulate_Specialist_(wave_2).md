# Swarm Simulate Specialist (wave 2)

- capability: `simulate`
- wave: 2
- run: `dba6ab7c-88a6-427f-8757-8411ed8c203c`

## Task (user turn)

Simulate what-if changes to weather, vehicle type, region or delivery mode and report how delay severity shifts. Scenario: 'storm in the East' meaning weather_condition=stormy for region=East (keep other fields same). Use today's predicted delayed orders as baseline. Apply changes to rows matching region=East and weather_condition currently not stormy (or all East rows if easier). Re-score severity against historical patterns and save simulation CSV.

Filters: select rows where region == 'East'.
Changes: set weather_condition='stormy'.

Return simulation results including how many deliveries become delayed and severity distribution shift.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to the shared blackboard the full JSON result from simulate_order_delays (including any simulation CSV path and key before/after delay/severity stats). This is needed to answer the 'storm in the East' impact question.
