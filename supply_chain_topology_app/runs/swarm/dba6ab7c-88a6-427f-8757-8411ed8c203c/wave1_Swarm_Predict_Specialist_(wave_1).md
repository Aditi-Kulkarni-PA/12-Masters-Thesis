...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the delay prediction pipeline for today’s orders from the provided raw input file, and post the resulting dataset of today’s predicted delayed orders with per-order severity classifications to the shared blackboard. This is the required starting point for diagnosing delay patterns, running the storm (East) what-if simulation, and producing optimization recommendations.
