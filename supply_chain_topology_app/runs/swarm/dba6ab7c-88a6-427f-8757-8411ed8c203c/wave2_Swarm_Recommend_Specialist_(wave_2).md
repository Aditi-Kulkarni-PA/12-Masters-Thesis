...
Produce data-driven optimization recommendations across quick-win, short-term and long-term horizons based on today's predicted delays and diagnosis results. Focus on mitigating the highest-risk delay patterns found today, and include actions for East during stormy weather if applicable.

Use the shared blackboard outputs from predict and diagnose (and simulation if available).
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to the shared blackboard the full JSON result from recommend (including prioritized actions and expected impact rationale). This is needed to answer 'tell me how to optimize delivery'.
