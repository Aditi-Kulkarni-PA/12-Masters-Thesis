...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders, including which orders are predicted to be delayed and their severity classifications, to the shared blackboard. This is needed because the request asks which orders can get delayed today, and downstream diagnosis, simulation, and customer email generation depend on today's predicted delay output.
