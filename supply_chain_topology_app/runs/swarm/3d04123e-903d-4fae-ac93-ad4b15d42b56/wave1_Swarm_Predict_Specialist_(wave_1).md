...
Predict which orders can get delayed today and classify each delayed order's severity. Also support downstream analysis for reasons and delay patterns, a what-if scenario, recommendations, and customer emails.

Scenario wording from the user: what will happen if there is a storm in the east region.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders and severity classifications to the shared blackboard, including enough identifying detail for downstream diagnosis, simulation, recommendations, and customer email generation. This is needed because the rest of the requested analyses depend on today's prediction output.
