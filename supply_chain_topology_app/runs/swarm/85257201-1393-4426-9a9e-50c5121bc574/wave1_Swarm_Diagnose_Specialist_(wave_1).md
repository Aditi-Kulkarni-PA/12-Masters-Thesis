...
- capability: `diagnose`
- wave: 1
- run: `85257201-1393-4426-9a9e-50c5121bc574`

## Task (user turn)

Find out which customer orders will be late and what are the reasons for the delay
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Using the predicted delayed orders posted to the blackboard by the predict step, analyze today’s delays against historical baselines across every relevant dimension and identify high-risk pattern combinations that explain why delays are happening. Post the diagnosis results (order-level or pattern-level reasons, plus the key contributing factors) to the shared blackboard so the delivery manager can see the reasons for delay.
