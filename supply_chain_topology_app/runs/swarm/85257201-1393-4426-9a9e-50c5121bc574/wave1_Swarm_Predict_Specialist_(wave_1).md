...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML pipeline over the raw input orders file provided in the task. Predict which of today’s orders will be delayed and classify each delayed order’s severity. Post the resulting dataset of today’s predicted delayed orders (including order identifiers and severity) to the shared blackboard so downstream diagnosis can explain the reasons.
