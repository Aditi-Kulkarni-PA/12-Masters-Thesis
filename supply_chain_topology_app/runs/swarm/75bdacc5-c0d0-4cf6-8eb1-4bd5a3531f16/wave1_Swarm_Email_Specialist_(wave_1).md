...
Before you finish, you MUST call write_blackboard to post your result. What to post: After predicted delayed orders with severity are available on the blackboard, generate severity-based customer email alerts: one personalized message per affected order. Ensure each email references the order’s severity level and is suitable for customer communication. Post the set of emails (mapped to affected order identifiers) to the shared blackboard for delivery/dispatch.
