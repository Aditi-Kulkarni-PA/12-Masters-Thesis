...
Generate severity-based customer email alerts, one personalised message per affected order. Works from today's predicted delayed orders.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to blackboard under capability 'email' the generated customer email alerts (one per affected order) and any counts/coverage metrics.
