...
Generate severity-based customer email alerts, one personalised message per affected order, using today's predicted delayed orders (with severity) from blackboard capability 'predict'. Include key reason/pattern and expected resolution window.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the generated emails to the blackboard under capability name 'email'.
