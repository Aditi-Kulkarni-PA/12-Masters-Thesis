...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Using today’s predicted delayed orders (with severity) posted to the shared blackboard, generate severity-based customer email alerts—one personalized message per affected order. Post the set of emails to the shared blackboard for delivery/dispatch, ensuring messages are appropriate to the predicted severity.
