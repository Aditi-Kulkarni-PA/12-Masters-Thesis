...
Analyze today's predicted delays against historical baselines across every dimension, and identify high-risk pattern combinations. Works from today's predicted delay output. Input: predicted delayed orders from blackboard capability 'predict'.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's diagnosis results (written delay-pattern diagnosis text with figures) to the blackboard under capability name 'diagnose'.
