...
Analyse today's predicted delays against historical baselines across every dimension, and identify high-risk pattern combinations. Use today's predicted delay output once it is available. The user wants delivery optimization recommendations, informed by today's diagnosis and the storm-in-the-East scenario.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's diagnosis results to the shared blackboard so recommend can read them.
