...
Before you finish, you MUST call write_blackboard to post your result. What to post: Analyze today’s predicted delays against historical baselines across every dimension and identify high-risk pattern combinations. Post the diagnosis results to the shared blackboard (e.g., which factors/patterns explain the delays and where risk concentrates) so recommendations and simulation can reference them.
