...
Before you finish, you MUST call write_blackboard to post your result. What to post: Generate severity-based customer email alerts: one personalized message per affected order predicted to be delayed. Use today’s predicted delayed orders (and severity) to tailor the tone/urgency. Post the email drafts (mapped to the affected orders) to the shared blackboard so the delivery manager can send them.
