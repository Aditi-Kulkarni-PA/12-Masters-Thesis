...
Before you finish, you MUST call write_blackboard to post your result. What to post: Simulate a what-if scenario: storm in the east region. Using today’s predicted delayed orders, re-score severity under the storm condition and report how delay severity shifts for the affected subset. Post the simulation outputs to the shared blackboard so the delivery optimization recommendations and customer email alerts can incorporate the storm impact.
