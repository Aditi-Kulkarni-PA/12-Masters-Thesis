...
- capability: `predict`
- wave: 1
- run: `d1bed37d-d663-40c4-bb18-d7c0abcf3376`

## Task (user turn)

Which of today's orders are predicted to be delayed?
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML pipeline over the raw input orders file provided in the task, and predict which of today's orders will be delayed and classify each one's severity. Post the resulting list of today's predicted delayed orders (including order identifiers and severity classification) to the shared blackboard so downstream steps (diagnose/recommend/email) can use it.
