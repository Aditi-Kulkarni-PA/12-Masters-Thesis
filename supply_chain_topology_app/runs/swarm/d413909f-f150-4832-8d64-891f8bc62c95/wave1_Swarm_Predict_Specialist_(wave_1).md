...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML prediction pipeline on the provided raw input orders file and post today’s predicted delayed orders with severity classifications to the shared blackboard, so downstream diagnosis and recommendations can use them.
