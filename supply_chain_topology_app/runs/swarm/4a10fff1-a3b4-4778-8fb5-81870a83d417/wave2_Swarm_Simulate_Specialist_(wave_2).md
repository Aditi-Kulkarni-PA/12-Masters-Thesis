...
Simulate what-if changes to weather for the East region using today's predicted delayed orders already produced from the input orders file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv. User scenario wording to preserve verbatim in intent: would that get worse if a storm hit the East? Apply an East-region storm scenario over today's predicted delayed orders and report how delay severity shifts.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the East storm what-if simulation results to the blackboard, including scenario definition, affected-row counts, and severity-shift outcomes. This is needed because the user explicitly asked whether delays would get worse if a storm hit the East.
