...
Predict which of today's orders will be delayed and classify each order's severity. Then make the predicted delayed orders available for downstream analysis of delay patterns and a what-if storm scenario in the East.
...
User request details to preserve for downstream specialists: What patterns are driving today's delays, and would that get worse if a storm hit the East?
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders and severity classifications to the shared blackboard, including enough summary context for downstream diagnosis and simulation to use them. This matters because both the pattern diagnosis and the East storm what-if simulation depend on the prediction output.
