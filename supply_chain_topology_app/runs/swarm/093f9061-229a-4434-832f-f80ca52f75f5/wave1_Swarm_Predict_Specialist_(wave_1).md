...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the delay prediction pipeline on the provided raw daily orders file and produce today’s predicted delayed orders with severity classification for each affected order. Post the resulting predictions (including order identifiers and predicted severity) to the shared blackboard so downstream diagnosis and customer email alerting can use it.
