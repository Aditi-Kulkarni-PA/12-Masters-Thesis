...
- capability: `email`
- wave: 1
- run: `093f9061-229a-4434-832f-f80ca52f75f5`
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Generate severity-based customer email alerts (one personalized message per affected order) using today’s predicted delayed orders. Post the draft emails to the shared blackboard keyed by order identifier so they can be reviewed/sent by the delivery manager. (If you cannot generate without predictions, request the needed prediction output from the blackboard.)
