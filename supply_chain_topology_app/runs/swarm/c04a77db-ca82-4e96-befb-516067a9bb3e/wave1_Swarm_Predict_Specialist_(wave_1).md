...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders, including each affected order and its predicted delay severity, to the shared blackboard so downstream specialists can use it. Also include any model-derived delay indicators that help explain why each order is likely late.
