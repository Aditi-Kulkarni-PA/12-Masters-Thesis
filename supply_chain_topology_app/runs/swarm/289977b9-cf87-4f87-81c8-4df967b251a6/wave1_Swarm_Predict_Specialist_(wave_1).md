...
- capability: `predict`
- wave: 1
- run: `289977b9-cf87-4f87-81c8-4df967b251a6`
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML prediction pipeline on the provided raw input orders file and produce the list of today’s orders predicted to be delayed, including each affected order’s predicted severity classification. Post the resulting predicted delayed orders (with severity per order) to the shared blackboard so downstream steps (diagnose/recommend/email) can use it.
