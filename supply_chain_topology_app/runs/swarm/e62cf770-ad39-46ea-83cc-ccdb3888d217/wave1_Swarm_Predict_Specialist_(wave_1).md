...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML pipeline over the raw input orders file supplied in the request, to predict which of today's orders will be delayed and classify each one's severity. Post the resulting list of today's predicted delayed orders (with severity classification) to the shared blackboard so downstream steps (diagnose, recommend/simulate if needed, and email) can use it.
