# Swarm Recommend Specialist (wave 2)
...
Provide data-driven recommendations to optimize order delivery today, using today's predicted delays and the diagnosis results already posted to the blackboard. Focus on quick-win, short-term, and long-term actions. Include specific levers tied to the diagnosis: delivery_mode (express), weather_condition (rainy/foggy/stormy), distance_category (long), and high-risk patterns (e.g., 'express + stormy', 'express + long', 'same day + stormy').
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post recommendations to the blackboard as JSON with sections quick_win/short_term/long_term, each containing bullet actions with expected impact (delay-rate reduction directionally) and which segments/patterns they target.
