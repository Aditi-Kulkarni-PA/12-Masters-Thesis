...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Run the two-stage ML pipeline over the raw input orders file at the provided path and predict which of today’s orders will be delayed and classify each delayed order’s severity. Post the resulting dataset/artifacts for “today’s predicted delayed orders” to the shared blackboard so downstream steps (diagnose/simulate/recommend/email) can use it. Include enough fields to identify orders and their predicted severity.
