...
Generate severity-based customer emails for orders predicted to be delayed today. Use the predicted delayed orders output and include personalization per affected order. Also include a short note referencing likely delay drivers (e.g., express + stormy/rainy/foggy, long distance) when available in the rule-based delay_reason or llm_insights. Input orders file path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post generated emails to the blackboard as JSON array with fields: order_id, predicted_severity_label, delay_reason, email_subject, email_body. If too many orders, include top N by severity (Long first) and mention truncation.
