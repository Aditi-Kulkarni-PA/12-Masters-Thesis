...
Simulate what-if changes to weather, vehicle type, region or delivery mode and report how delay severity shifts. Scenario: storm in the east region. Use today's predicted delayed orders as baseline. Apply weather_condition='stormy' and region='east' (keep other fields as-is). Return simulation CSV and summary of how delay severity shifts for affected orders. Input orders file path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post the simulation result to the blackboard with key fields: scenario, affected_filters, changes, predicted_delay_rate_shift (if available), and top affected segments (e.g., delivery_mode, vehicle_type, distance_category) so downstream recommendations and emails can reference it.
