...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: After today’s predicted delayed orders are available on the blackboard, run a what-if simulation for: storm in the east region. Re-score severity for the affected orders using the historical severity patterns already available. Post the simulation output (how delay severity shifts under storm-in-east) to the shared blackboard so the manager can understand what will happen under that scenario.
