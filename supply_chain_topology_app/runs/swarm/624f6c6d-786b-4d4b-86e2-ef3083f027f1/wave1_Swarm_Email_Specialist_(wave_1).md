...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: After today’s predicted delayed orders are available on the blackboard, generate severity-based customer email alerts: one personalized message per affected order. Post the email drafts to the shared blackboard, ensuring each email is tied to the correct order and predicted severity, and includes appropriate delivery-delay wording without exposing sensitive internal details.
