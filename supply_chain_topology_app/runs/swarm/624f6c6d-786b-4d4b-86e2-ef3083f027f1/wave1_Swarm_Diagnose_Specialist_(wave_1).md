# Swarm Diagnose Specialist (wave 1)
...
Which orders can get delayed today, what are the reasons and delay patterns, what will happen if there is a storm in the east region, provide your recommendations to optimize the order delivery and also provide emails that can be sent to customers whose orders are predicted to be delayed
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: After the predicted delayed orders are available on the blackboard, analyze today’s predicted delays against historical baselines across every dimension and identify high-risk pattern combinations (reasons/patterns). Post the diagnosis results to the shared blackboard, clearly mapping which pattern combinations correspond to which predicted severity/segments so recommendations and simulation can reference them.
