...
Run the two-stage ML pipeline over the raw input orders file supplied in the request.
...
Return today's predicted delayed orders with severity classification.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post to blackboard under capability 'predict' the predicted delayed orders (order ids), severity, and any key features used (mode, region, weather, vehicle, partner, distance bucket) so downstream diagnose/recommend/email can cite hotspots.
