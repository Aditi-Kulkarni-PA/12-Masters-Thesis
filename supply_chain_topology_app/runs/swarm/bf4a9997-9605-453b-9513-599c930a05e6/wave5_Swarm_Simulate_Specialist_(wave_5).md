# Swarm Simulate Specialist (wave 5)
...
Run what-if simulations using today's predicted delayed orders to test how delay severity shifts under targeted operational changes suggested by the recommendation output. Prioritize scenarios for: (1) converting express + long (>200 km) orders to two-day mode, (2) replacing bikes/EV bikes/scooters with enclosed vehicles on stormy and rainy urgent lanes, and (3) shifting urgent allocations away from XpressBees and Ekart toward Tier 1 partners where applicable. The input orders data is in the file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post simulation results to the blackboard showing severity changes for each scenario, because predict output and recommend output are now available and these scenarios are ready for downstream evaluation.
