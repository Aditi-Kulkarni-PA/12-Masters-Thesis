...
Determine which of today's orders are likely to be delayed and classify each delayed order's severity for this request: identify what patterns are driving today's delays, determine what should be done about it, and let affected customers know.
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders and severity classifications to the shared blackboard, because diagnosis and customer email alerts both depend on the predicted delay output, and recommendations may also need it downstream.
