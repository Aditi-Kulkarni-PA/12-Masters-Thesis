...
Produce data-driven optimization recommendations across quick-win, short-term and long-term horizons using today's predicted delay output already produced from the input orders file at path: /Users/aditikulkarni/Documents/Masters/AI-Projects/12-Thesis/0_supply_chain_thesis/prediction_pipeline/data/raw/daily_delivery_logistics_1.csv and today's diagnosis results. Focus on the reasons behind late orders and the root-cause patterns behind the predicted delays.
...
Before you finish, you MUST call write_blackboard to post your result. What to post: Post recommendation results to the blackboard, using today's predicted delay output together with today's diagnosis results to produce actionable optimization recommendations. This is needed because the user asked for reasons behind late orders and what to do about them.
