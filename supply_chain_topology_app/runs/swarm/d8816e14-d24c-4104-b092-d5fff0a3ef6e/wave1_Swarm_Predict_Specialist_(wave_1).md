...
Predict which of today's orders will be delayed and classify each order's severity for this request about delivery impact if there is a storm in the east today.
...
Scenario wording to carry forward exactly: What will be the delivery impact if there is a storm in the east today?
...
## Blackboard
Before you finish, you MUST call write_blackboard to post your result. What to post: Post today's predicted delayed orders and severity classifications to the shared blackboard, including enough summary detail for downstream specialists to identify affected east-region orders under the storm scenario. This matters because simulation and any downstream customer-impact work depend on today's prediction output.
