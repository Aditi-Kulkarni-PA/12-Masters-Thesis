...
from helpers.app_utils import is_error_text
from core.paths import PREDICT_CSV, PIPELINE_PROCESSED, EMAIL_ALERTS_CSV

# Pipeline's processed directory (source of truth for predict outputs).
# From core.paths, not recomputed here — see that module for why.
...
def process_predict(
    ...
    if fmt_stats and not is_error_text(predict_text):
...
def process_emails(
    ...
    # app_dir.parent IS the repo root. This was app_dir.parent.parent, one level too
    # high, so it pointed outside the project entirely and silently never matched —
    # the same off-by-one that made email_customers.py read a stale CSV.
    predict_csv = app_dir / "output" / "daily_delivery_delay_prediction.csv"
    # Prefer the email capability's own artifact; fall back to predict's output
    # for a run where email never executed.
    pipeline_csv = EMAIL_ALERTS_CSV if EMAIL_ALERTS_CSV.is_file() else PREDICT_CSV
