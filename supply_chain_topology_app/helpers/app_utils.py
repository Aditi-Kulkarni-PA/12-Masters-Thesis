...
#----- Surface Errors and Dont Show Results if the tool/pipeline failed to run (e.g. upstream missing) -----
# Matched with word boundaries, not bare substrings. A bare "error" substring
# swallowed legitimate narratives ("reduced error rate", "margin of error"): the tab
# was then silently withheld and the run looked like it produced nothing. Errors are
# recognised by the shapes tools actually emit -- a leading "ERROR", the
# {"Error": "upstream_missing", ...} payload from prediction_server.py, or an explicit
# failure phrase -- so ordinary prose containing the word "error" no longer trips it.
_ERROR_PATTERNS = (
    r'^\s*(?:#{1,6}\s*)?\**\s*error\b',      # text opens with ERROR (optionally after ## / **)
    r'["\']error["\']\s*:',                  # a JSON error payload, any casing
    r'\bupstream[_ ]missing\b',              # underscore in the real payload; space kept for prose
    r'\bpipeline\s+failed\b',
    r'\bfailed\s+to\s+(?:run|execute)\b',
    r'\bunable\s+to\s+(?:run|complete)\b',
    r'\btool\s+(?:failed|error|crashed|exception|timeout)\b',
    r'\b(?:did|does|has|have|was|were|could)\s+not\s+run\b',
    r'\bno\s+run\b',
)
...
def is_error_text(text: str) -> bool:
    """True if *text* reads as a reported tool/pipeline error rather than real output."""
    if not text:
        return False
