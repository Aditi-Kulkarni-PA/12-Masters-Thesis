...
from measurement.dependencies import scheduling_values
...
def compute_for_run(conn: sqlite3.Connection, run_id: str) -> dict | None:
    """Return the scheduling values for one run, or None when undefined.

    Reads the run's stored tool_call rows and hands them to
    measurement.dependencies.scheduling_values(), which is the single implementation
    shared with the write path. This function only supplies the input; it does no
    arithmetic of its own, so a backfilled run and a run written today cannot disagree.
    ...
    return scheduling_values(calls)
