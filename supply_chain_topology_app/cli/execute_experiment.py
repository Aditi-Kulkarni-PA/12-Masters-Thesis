"""
Experiment harness (T34): plans one repetition -- every topology x query pair once --
and executes each one by invoking scripts/execute_topology.sh, so every run still goes
through that script's existing measurement-mode guardrails (NO_CACHE, dev-path
fallback off) unchanged.

Every run is stamped with the experiment it belongs to, where an experiment is one
run_phase at one model (measurement/run_store_schema.py, table `experiment`). The
number is looked up rather than typed: give the harness the phase and the model and it
resolves the rest.

Run from the repo root, same convention as execute_topology.sh:

    python .../execute_experiment.py --run-phase main --model gpt-5.4 --run-n 1
    python .../execute_experiment.py -e 4 --run-n 2
    python .../execute_experiment.py -e 4 --run-n 2 -q 1,2,3
    python .../execute_experiment.py -e 4 --run-n 2 --dry-run
    python .../execute_experiment.py --list
...
(run_phase, model, topology, query_id, run_n) slot that already has a success/partial run is skipped,
...
from measurement.run_store_schema import (
    DB_PATH, UnknownExperimentError, experiment_def, experiment_for)
...
def resolve_experiment(args) -> tuple[int | None, str, str]:
    """Settle which campaign this invocation belongs to. Returns (experiment_no, run_phase, model).

    Two accepted forms, mutually exclusive:
      -e N                          cite an existing experiment; its phase and model are
                                    read from the experiment table
      --run-phase P --model M       name the pair; the number is looked up, or allocated
                                    when --new-experiment is also given

    The resolved model is then compared against the MODEL this process actually has, so
    a stale .env cannot file runs under a tier they were not executed at.

    A --dry-run never allocates: the number comes back None and the caller reports that
    one would be opened. Writing a row while previewing would leave an empty campaign
    behind every time a plan was inspected and discarded.
    """
    if args.experiment is not None:
        if args.run_phase or args.model:
            raise SystemExit(
                "ABORT: -e/--experiment names a campaign that already has a phase and a "
                "model. Pass either -e, or --run-phase with --model, not both.")
        if args.new_experiment:
            raise SystemExit(
                "ABORT: --new-experiment allocates a number, so it cannot be combined "
                "with -e, which cites one. Use --run-phase with --model.")
        try:
            run_phase, model = experiment_def(args.experiment, DB_PATH)
        except UnknownExperimentError as exc:
            raise SystemExit(f"ABORT: {exc}")
        experiment_no = args.experiment
    else:
        missing = [f for f, v in (("--run-phase", args.run_phase), ("--model", args.model))
                   if not v]
        if missing:
            raise SystemExit(
                f"ABORT: {' and '.join(missing)} required. Name the campaign as "
                f"--run-phase <pilot|main> --model <model>, or cite an existing one with "
                f"-e N (list them with run_store_schema.py --list-experiments).")
        run_phase, model = args.run_phase, args.model
        try:
            experiment_no = experiment_for(run_phase, model, DB_PATH,
                                           create=args.new_experiment and not args.dry_run)
        except UnknownExperimentError as exc:
            if args.new_experiment and args.dry_run:
                experiment_no = None          # would be allocated on a real invocation
            else:
                raise SystemExit(f"ABORT: {exc}")

    # The model is supplied on the command line and resolved from .env independently.
    # Requiring both to agree turns a silent contamination into a refusal: a batch run
    # against the wrong tier produces rows that look valid and are not.
    if model != MODEL:
        raise SystemExit(
            f"ABORT: experiment {experiment_no} is at model {model!r}, but this process "
            f"resolves OPENAI_MODEL to {MODEL!r}.\n"
            f"  Check  : grep -E '^OPENAI_MODEL=' .env\n"
            f"  Nothing was executed. A batch under a mismatched tier cannot be "
            f"distinguished from a valid one after the fact.")
    return experiment_no, run_phase, model


# ---------------------------------------------------------------------------
# Resume: (run_phase, model, topology, query_id, run_n) tuples that already have a
# usable run, so a re-invocation after a stop/crash does not create a duplicate run_n
# for the same slot (the store has no uniqueness constraint on it -- this check is what
# keeps it unique in practice).
#
# model is part of the key because a model switch between batches (e.g. gpt-5.4 ->
# gpt-5.4-nano) must re-run every combo, not silently reuse an old model's row. Found
# 6-Sep-26: the pilot's first nano dry-run planned 74 of 99 slots because 25 combos
# already had a success/partial row from gpt-5.4 runs collected 29/30-Aug-26.
#
# run_phase is part of it for the same reason one level up. The pilot is N=1 at every
# tier, so a main-experiment run_n=1 at the frozen tier matches its pilot row on
# topology, query, run_n and model alike -- resume would skip the entire first
# repetition of the main experiment and report nothing to do.
# ---------------------------------------------------------------------------
def already_done(db_path: str) -> set[tuple[str, str, str, str, int]]:
    if not Path(db_path).exists():
        return set()
    conn = sqlite3.connect(db_path)
    try:
        rows = conn.execute(
            "SELECT run_phase, model, topology, query_id, run_n FROM run "
            "WHERE run_status IN ('success','partial')"
        ).fetchall()
    finally:
        conn.close()
    return {(p, m, t, q, n) for p, m, t, q, n in rows}


def build_plan(topologies: list[str], queries: list[str], run_n: int,
              resume: bool, shuffle: bool, db_path: str, model: str,
              run_phase: str) -> list[dict]:
    """One planned run per topology x query pair, all at the same *run_n*.

    Pairs already done in THIS experiment at THIS run_n are dropped when *resume* is
    set, so re-invoking after a stop continues rather than duplicating."""
    done = already_done(db_path) if resume else set()
    plan = [
        {"topology": t, "query_id": q, "run_n": run_n}
        for t in topologies for q in queries
        if (run_phase, model, t, q, run_n) not in done
    ]
    if shuffle:
        random.shuffle(plan)
...
def run_one(item: dict, batch_id: str, run_phase: str, experiment_no: int | None, log_dir: Path,
    ...
    The topology, query and run_n go as -t/-q/-n flags, not env vars: execute_topology.sh
    exports SC_TOPOLOGY, SC_QUERY_ID and SC_RUN_N from its own flags, so a value of the
    same name inherited from this process would be overwritten and silently ignored. The
    campaign variables below have no flag equivalent and are passed through the
    environment.
    ...
    env.update({
        ...
        "SC_EXPERIMENT_NO": str(experiment_no) if experiment_no is not None else "",
    ...
    with open(log_path, "w", encoding="utf-8") as logf:
        proc = subprocess.Popen(
            ["bash", str(_EXECUTE_SCRIPT),
             "-t", item["topology"], "-q", item["query_id"], "-n", str(item["run_n"])],
        ...
        try:
            ...
            logf.write(f"\n\n[execute_experiment.py] TIMEOUT after {timeout_s}s -- "
...
def _record_non_completion(item: dict, batch_id: str, run_phase: str, experiment_no: int | None,
    ...
    try:
        write_failed_run(
            ...
            run_phase=run_phase, experiment_no=experiment_no, batch_id=batch_id,
...
def main() -> int:
    ...
    # The campaign, named either by number or by the pair it is defined as. Neither has
    # a default: a wrong phase or tier is not recoverable from the rows afterwards, so
    # this is stated on every invocation rather than inherited from anywhere.
    ap.add_argument("-e", "--experiment", type=int, default=None,
                    help="experiment number to run under (see run_store_schema.py "
                         "--list-experiments). Alternative to --run-phase with --model")
    ap.add_argument("--run-phase", default=None,
                    help="pilot | main. Use with --model instead of -e")
    ap.add_argument("--model", default=None,
                    help="model tier this campaign runs at. Must match OPENAI_MODEL in "
                         ".env or the batch aborts before spending anything")
    ap.add_argument("--new-experiment", action="store_true",
                    help="allocate a number for a (run-phase, model) pair that has none "
                         "yet. Required the first time a campaign is opened")
    ap.add_argument("--run-n", type=int, default=None,
                    help="which repetition this is. Every run in the batch carries it, "
                         "so N=3 means three invocations with --run-n 1, 2, 3")
    ...
    # Required for anything that plans or executes runs; --list exits before this.
    if args.run_n is None:
        raise SystemExit("ABORT: --run-n is required. It is the repetition number every "
                         "run in this batch carries, so a resumed batch lands in the same "
                         "slot rather than beside it.")
    experiment_no, run_phase, model = resolve_experiment(args)
    run_n = args.run_n
    batch_id = args.batch or f"batch_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
    resume = not args.no_resume

    plan = build_plan(topologies, queries, run_n, resume=resume,
                      shuffle=not args.no_shuffle, db_path=DB_PATH, model=model,
                      run_phase=run_phase)

    _exp_label = experiment_no if experiment_no is not None else "(would be allocated)"
    print(f"Batch {batch_id}: {len(plan)} run(s) planned "
          f"(experiment {_exp_label} = {run_phase}/{model}, run_n={run_n}: "
          f"{len(topologies)} topolog(ies) x {len(queries)} quer(ies), "
    ...
    try:
        for item in plan:
            ...
            ok, log_path, hit_timeout = run_one(item, batch_id, run_phase, experiment_no,
                                                batch_log_dir, timeout_s=args.run_timeout)
            if hit_timeout:
            ...
            if not ok:
                _record_non_completion(item, batch_id, run_phase, experiment_no, log_path,
