...
# torch's own JIT-overload registration (triggered transitively by sentence_transformers
# importing transformers -> torch) emits a cosmetic UserWarning on some environments --
# "Unable to retrieve source for @torch.jit._overload function: tensordot/norm". It does
# not affect correctness; TorchScript just skips deeper introspection of that function
# and proceeds normally. Silenced per user request (23-Aug-26) -- must be set BEFORE the
# sentence_transformers import below, since that's what triggers it.
...
def _get_cross_encoder() -> CrossEncoder:
    ...
    Progress bars are suppressed at process start (see execute_topology.py) — the env vars
    must be set before sentence_transformers is imported, not here. The singleton is
    process-scoped, so it reloads once per run; from the local HF cache that is well
    under a second and constant across topologies, so it does not bias the comparison.
    """
...
def _get_collection() -> chromadb.Collection:
    ...
    if needs_rebuild:
        ...
        _LOGGER.info("rag.vectorstore.building source=%s", _SLA_FILE.name)
        ...
        _LOGGER.info("rag.vectorstore.indexed chunks=%s", len(chunks))
