...
from core.paths import PIPELINE_DB, PIPELINE_PROCESSED

# ---------------------------------------------------------------------------
# Paths
# ---------------------------------------------------------------------------
_BASE = Path(__file__).resolve().parent.parent.parent  # 0_supply_chain_thesis
_DB_PATH = PIPELINE_DB
_CSV_DIR = PIPELINE_PROCESSED
...
# Today's diagnosis narrative runs 2-4k characters in every observed real run. A
# coordinator that has not actually obtained a diagnosis, but wants to satisfy the
# parameter, tends to pass a placeholder ("n/a", "none", "not available") rather than
# leave it empty -- so an emptiness check alone would not enforce the dependency it
# exists to enforce. 40 characters sits far above any such placeholder and far below
# any real diagnosis, so it separates the two without judging content.
_MIN_DIAGNOSIS_CHARS = 40


@tool
def recommend_actions(diagnosis_summary: str) -> str:
    """Produce data-driven optimization recommendations across quick-win, short-
    term and long-term horizons. Works from today's predicted delay output
    together with today's diagnosis results, and from retrieved service-
    level knowledge.
    ...
    Args:
        diagnosis_summary: today's written delay-pattern diagnosis -- the
            root-cause reading of which dimensions, partners and pattern
            combinations are driving today's delays against historical
            baselines. This is the diagnosis capability's own produced output.
            The recommendations returned here are grounded in it, so it is
            required input, not optional context.
    """
    ...
    try:
        ...
        # Today's diagnosis is a genuine INPUT to this capability, not merely work that
        # happens to have run earlier: sections 9b-9d below read it directly. Reported
        # in the same upstream_missing shape the pipeline tools use
        # (prediction_server.py), so every condition's existing dependency-recovery
        # path (shared/dependency_discovery.md, self_check.md) handles it unchanged.
        if len((diagnosis_summary or "").strip()) < _MIN_DIAGNOSIS_CHARS:
            status = "upstream_missing"
            _LOGGER.warning("tool.recommend_actions.upstream_missing chars=%s",
                            len((diagnosis_summary or "").strip()))
            return json.dumps({
                "Error": "upstream_missing",
                "message": (
                    "Today's delay-pattern diagnosis has not been produced yet. "
                    "Recommendations are grounded in it and cannot be produced "
                    "without it. Obtain today's diagnosis results, then supply them "
        ...
        # --- 9pre. Today's diagnosis, as produced by the diagnosis capability ---
        # Placed FIRST so it frames every figure below it rather than reading as an
        # appendix. The structured DB sections that follow are deliberately kept
        # (23-Aug-26 decision): the narrative carries the reasoning, but recommendations
        # must cite exact figures in supporting_data, and prose alone is a weaker
        # grounding source than the numbers themselves.
        sections.append("## Today's Delay Diagnosis (produced by the diagnosis capability)")
        sections.append(diagnosis_summary.strip())
        sections.append("")
        ...
        _LOGGER.info(
            "tool.recommend_actions.data diagnosis_chars=%s dims_compared=%s hist_high_risk=%s "
            "daily_high_risk=%s severity_hotspots=%s hist_top_dims=%s daily_top_dims=%s output_chars=%s",
            len(diagnosis_summary.strip()), len(dim_comparisons), len(hist_risk), len(daily_risk),
