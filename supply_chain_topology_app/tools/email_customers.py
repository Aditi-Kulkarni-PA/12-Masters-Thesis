...
from core.paths import APP_OUTPUT_CSV, PREDICT_CSV, EMAIL_ALERTS_CSV

_TOOL_DIR = Path(__file__).resolve().parent
_APP_DIR = _TOOL_DIR.parent                       # supply_chain_delivery_app/
_OUTPUT_CSV = APP_OUTPUT_CSV
# Pipeline's canonical CSV (always fresh after a predict run in the same session)
# _APP_DIR.parent is the repo root. This was .parent.parent, one level too high, so it
# pointed OUTSIDE the project and never existed -- the tool silently fell back to the
# app output copy, or to whatever stale file happened to sit at the wrong path.
_PIPELINE_CSV = PREDICT_CSV
...
def fetch_delayed_orders_for_email() -> str:
    """Generate severity-based customer email alerts, one personalised message per
    affected order. Works from today's predicted delayed orders.
    ...
    try:
        ...
        # Write to the email capability's OWN artifact, never back over the source.
        # csv_path is predict's canonical output, which every other capability reads;
        # writing a capped delayed-only subset over it silently destroyed 1,346 rows
        # down to 143 and left the run looking successful.
        if "predict_delay" in df.columns:
            ...
            out = delayed
        EMAIL_ALERTS_CSV.parent.mkdir(parents=True, exist_ok=True)
        out.to_csv(EMAIL_ALERTS_CSV, index=False)

        _LOGGER.info("tool.fetch_delayed_orders_for_email.generated count=%s", len(email_texts))
