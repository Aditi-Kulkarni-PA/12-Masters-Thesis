...
class DynamicGraphCoordinator:
    ...
    async def _triage_turn(self, message: str) -> AgentResponse:
        """Classify the request and record whether to act on it. Sets _proceed."""
        self._triaged = True
        self._query = message
        triage = Agent(
    ...
    async def run(self, message: str, *, session=None, middleware=None) -> AgentResponse:
        if not self._triaged:
            return await self._triage_turn(message)
        ...
        if not self._proceed:
            if not getattr(self, "_retried", False):
                ...
                # instead returns the triage response, because triage is turn-1 behaviour
                # and execution belongs to the turn after it -- there is no turn after
                # this one. Observed 8-Sep-26 on static_graph_routed: the retry re-decided
                # correctly, reported proceed=true with a three-capability plan, then
                # returned it and ran nothing.
                self._retried = True
                self._triaged = False
                combined = f"{self._query}\n\n{message}" if self._query else message
                await self._triage_turn(combined)
                if not self._proceed:
                    return self._final
            else:
