<!--
RETIRED — NOT LOADED BY ANY TOPOLOGY. DO NOT WIRE THIS FILE INTO A MEASUREMENT RUN.

This is the pre-topology baseline app's master prompt, kept only as the historical
comparison point several READMEs cite (coordinators/planner_executor/README.md,
shared/README.md, prompts/README.md) and deliberately exempted from the prompt gate
(helper/validate_specs.py). Every live coordinator builds from
coordinators/<topology>/master.md instead.

Two reasons it must stay retired rather than be reused:

1. It HARDCODES CALL ORDER in seven places ("Order: predict -> diagnose -> simulate ->
   recommend -> email_alert", "call X first, then Y"). Deciding that order is the
   behaviour under measurement, so a condition given it is not being measured -- it is
   being told the answer (Risk Log R13).

2. Its dependency claims were WRONG when written. It states "The recommendation tool
   reads prediction and diagnosis results from the DB", asserting a diagnose->recommend
   data dependency that did not exist: get_delay_diagnosis wrote nothing, and
   recommend_actions() independently re-queried the same tables. That dependency was
   made real on 23-Aug-26 by changing the code (recommend_actions now requires a
   diagnosis_summary argument) -- see Multi_Agent_Topology_Reference.md section 7 -- but
   the ordering language here is still wrong for the reason in (1) regardless.
-->
