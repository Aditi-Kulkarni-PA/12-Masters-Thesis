...
| UI | COMPLETE | Gradio 5-tab conversational interface with quick actions — Section 5, Section 6 |
...
A self-contained Python module responsible for all machine learning concerns: data ingestion, feature engineering, model training, daily inference, and database writes. It exposes its capabilities to the agent layer through a FastMCP server over stdio transport — see [`docs/supply-chain-app/06-mcp-server-design.md`](docs/supply-chain-app/06-mcp-server-design.md) for tool contracts and design decisions.
...
> **[`docs/supply-chain-app/01-architecture.md`](docs/supply-chain-app/01-architecture.md)**
...
> **[`docs/supply-chain-app/02-tech-stack.md`](docs/supply-chain-app/02-tech-stack.md)**
...
│       (ml-pipeline-doc.ipynb → see docs/supply-chain-app/04-ml-pipeline-architecture.md)
│
...
> Full iteration details, design rationale, and prompt examples: **[`docs/supply-chain-app/03-prompt-evolution-log.md`](docs/supply-chain-app/03-prompt-evolution-log.md)**

### Per-Agent Workflow Documents

- **[`docs/supply-chain-app/08-master-agent-flowchart.md`](docs/supply-chain-app/08-master-agent-flowchart.md)** — Master agent flow: plan confirmation gate, sequential tool execution, freshness detection; Fallback Advisor: trigger conditions, WebSearchTool usage, design rationale
- **[`docs/supply-chain-app/09-agent-predict-workflow.md`](docs/supply-chain-app/09-agent-predict-workflow.md)** — Predict agent flow: schema slimming, sidecar JSON, few-shot examples
- **[`docs/supply-chain-app/10-agent-diagnose-workflow.md`](docs/supply-chain-app/10-agent-diagnose-workflow.md)** — Diagnose agent flow: freshness gate design
- **[`docs/supply-chain-app/11-agent-simulate-workflow.md`](docs/supply-chain-app/11-agent-simulate-workflow.md)** — Simulate agent flow: 3-arg tool, FRESH/NOT FRESH prerequisite
- **[`docs/supply-chain-app/12-agent-recommend-workflow.md`](docs/supply-chain-app/12-agent-recommend-workflow.md)** — Recommend agent flow: SLA citation, category enforcement, RAG grounding
- **[`docs/supply-chain-app/13-agent-email-workflow.md`](docs/supply-chain-app/13-agent-email-workflow.md)** — Email agent flow: severity-specific templates, single EmailAlert pattern
- **[`docs/supply-chain-app/15-security-guardrails-design.md`](docs/supply-chain-app/15-security-guardrails-design.md)** — Security guardrails: why they live only on the Master agent (Fallback Advisor coverage included)
...
> Full tool contracts, stdio transport architecture, `tool_filter` design, connection setup, and timeout rationale: **[`docs/supply-chain-app/06-mcp-server-design.md`](docs/supply-chain-app/06-mcp-server-design.md)**
...
> Tool contracts, parameter schemas, and transport design: **[`docs/supply-chain-app/06-mcp-server-design.md`](docs/supply-chain-app/06-mcp-server-design.md)**
...
> Full table inventory, column schema, engineered features, high-risk pattern classification, agent read patterns, and write lifecycle: **[`docs/supply-chain-app/05-sqlite-database-design.md`](docs/supply-chain-app/05-sqlite-database-design.md)**
...
> Full pipeline design, chunking strategy, hybrid scoring formula, cross-encoder rationale, retrieval cache, and output format: **[`docs/supply-chain-app/07-vectorstore-rag-design.md`](docs/supply-chain-app/07-vectorstore-rag-design.md)**  
> Per-agent usage in context: **[`docs/supply-chain-app/12-agent-recommend-workflow.md`](docs/supply-chain-app/12-agent-recommend-workflow.md)**
...
> Full design, sidecar file locations, cache key composition, eviction policy, and ChromaDB rebuild detection: **[`docs/supply-chain-app/16-caching-design.md`](docs/supply-chain-app/16-caching-design.md)**
...
> Full event reference, log format, audit sidecar schemas, output file inventory, and tracing config: **[`docs/supply-chain-app/17-observability-logging.md`](docs/supply-chain-app/17-observability-logging.md)**
...
> Attack surface analysis, guardrail design rationale, and Fallback Advisor coverage: **[`docs/supply-chain-app/15-security-guardrails-design.md`](docs/supply-chain-app/15-security-guardrails-design.md)**
...
| # | Doc | Contents |
|---|---|---|
| 01 | [`docs/supply-chain-app/01-architecture.md`](docs/supply-chain-app/01-architecture.md) | Layered architecture diagram, agent inventory, links to all workflow docs |
| 02 | [`docs/supply-chain-app/02-tech-stack.md`](docs/supply-chain-app/02-tech-stack.md) | Full technology stack with library details and design comments |
| 03 | [`docs/supply-chain-app/03-prompt-evolution-log.md`](docs/supply-chain-app/03-prompt-evolution-log.md) | Prompt engineering iteration history across all 7 agents |
| 04 | [`docs/supply-chain-app/04-ml-pipeline-architecture.md`](docs/supply-chain-app/04-ml-pipeline-architecture.md) | ML training pipeline flow — data extract → clean → EDA → feature engineering → model selection → persistence |
| 05 | [`docs/supply-chain-app/05-sqlite-database-design.md`](docs/supply-chain-app/05-sqlite-database-design.md) | SQLite schema: 27 tables, summary table structure, high-risk pattern classification, write lifecycle |
| 06 | [`docs/supply-chain-app/06-mcp-server-design.md`](docs/supply-chain-app/06-mcp-server-design.md) | FastMCP server: 3 tool contracts, stdio transport design, connection setup, key design decisions |
| 07 | [`docs/supply-chain-app/07-vectorstore-rag-design.md`](docs/supply-chain-app/07-vectorstore-rag-design.md) | ChromaDB store, three-stage retrieval (cosine → hybrid → cross-encoder), chunking pipeline, retrieval cache |
| 08 | [`docs/supply-chain-app/08-master-agent-flowchart.md`](docs/supply-chain-app/08-master-agent-flowchart.md) | Master & Fallback agent workflows: plan confirmation gate, sequential tool execution, post-processing; Fallback Advisor trigger, WebSearchTool, design rationale |
| 09 | [`docs/supply-chain-app/09-agent-predict-workflow.md`](docs/supply-chain-app/09-agent-predict-workflow.md) | Predict agent flow diagram + all design fixes (sidecar JSON, schema slimming, Pydantic enforcement) |
| 10 | [`docs/supply-chain-app/10-agent-diagnose-workflow.md`](docs/supply-chain-app/10-agent-diagnose-workflow.md) | Diagnose agent flow diagram + freshness check design |
| 11 | [`docs/supply-chain-app/11-agent-simulate-workflow.md`](docs/supply-chain-app/11-agent-simulate-workflow.md) | Simulation agent flow diagram + severity lookup logic + design decisions |
| 12 | [`docs/supply-chain-app/12-agent-recommend-workflow.md`](docs/supply-chain-app/12-agent-recommend-workflow.md) | Recommend agent + three-stage RAG pipeline (ChromaDB → hybrid → cross-encoder) |
| 13 | [`docs/supply-chain-app/13-agent-email-workflow.md`](docs/supply-chain-app/13-agent-email-workflow.md) | Email agent flow + severity templates + Pydantic model |
| 14 | [`docs/supply-chain-app/14-gradio-ui-design.md`](docs/supply-chain-app/14-gradio-ui-design.md) | ChatInterface vs manual Chatbot — design rationale for 15-tuple yield approach |
| 15 | [`docs/supply-chain-app/15-security-guardrails-design.md`](docs/supply-chain-app/15-security-guardrails-design.md) | Attack surface analysis + why sub-agents don't need guardrails prepended |
| 16 | [`docs/supply-chain-app/16-caching-design.md`](docs/supply-chain-app/16-caching-design.md) | Sidecar freshness detection (1h TTL), RAG retrieval cache (SHA-256 key, 200-entry eviction), ChromaDB rebuild detection |
| 17 | [`docs/supply-chain-app/17-observability-logging.md`](docs/supply-chain-app/17-observability-logging.md) | Runtime logger, structured RAG events, audit sidecar files, output file inventory, agent tracing config |
| 18 | [`docs/supply-chain-app/18-end-to-end-data-flow.md`](docs/supply-chain-app/18-end-to-end-data-flow.md) | End-to-end data flow from CSV input through ML inference, SQLite, agent orchestration to UI output |
| 19 | [`docs/supply-chain-app/19-format-agent-design.md`](docs/supply-chain-app/19-format-agent-design.md) | Format Summary agent (replaced by deterministic formatting in `post_processing.py`; kept available): original Markdown rendering design |
| 20 | [`docs/supply-chain-app/20-executive-flow-diagrams.md`](docs/supply-chain-app/20-executive-flow-diagrams.md) | Executive-level flow diagrams for presentation and documentation |
| 21 | [`docs/supply-chain-app/21-eval-flow-design.md`](docs/supply-chain-app/21-eval-flow-design.md) | Eval framework: 5-agent LLM-as-judge design, RAGAS integration, DB isolation, run commands |
...
> Full eval framework design, DB isolation, fixture lifecycle, and per-agent rubric: **[`docs/supply-chain-app/21-eval-flow-design.md`](docs/supply-chain-app/21-eval-flow-design.md)**
...
> RAGAS eval implementation: **[`evals/test_eval_rag.py`](evals/test_eval_rag.py)** · Full design rationale: **[`docs/supply-chain-app/21-eval-flow-design.md`](docs/supply-chain-app/21-eval-flow-design.md)**
